...
def steps(refresh: bool, fetch_manifests: bool, manifest_limit: int) -> list[Step]:
    py = sys.executable
    brew_cmd = [py, "scripts/01-brew-fetch.py"]
    manager_cmd = [py, "scripts/02-manager-indexes.py"]
    if refresh:
    ...
    return [
        Step(
            "brew-fetch",
            brew_cmd,
            [Path("scripts/01-brew-fetch.py"), Path("scripts/lib/brew.py"), Path("scripts/lib/common.py"), Path("scripts/lib/executables.py")],
        ...
        Step(
            "manager-indexes",
            manager_cmd,
            [Path("scripts/02-manager-indexes.py"), Path("scripts/lib/managers.py"), Path("scripts/lib/common.py")],
            [Path("cache/pkg-manager-indexes.json.gz")],
            refresh_sensitive=True,
        ),
        Step(
            "render-projects",
            [py, "scripts/03-render-projects.py"],
            [
                Path("scripts/03-render-projects.py"),
                Path("scripts/lib/render.py"),
                Path("scripts/lib/yaml_writer.py"),
                Path("scripts/lib/brew.py"),
                Path("scripts/lib/managers.py"),
        ...
        Step(
            "publish-projects",
            [py, "scripts/04-publish-projects.py"],
            [
                Path("scripts/04-publish-projects.py"),
                Path("scripts/lib/render.py"),
